...
const groupData = async (i, url) => {
    ...
    for (let i = 0; i < response.data.products.length; i++) {
        ...
        // aTODO Do we need this?
        // if (categoryPathIsBanned(p.category.path)) {
        //     continue;
        // }
        ...
        await helpers.findPrice(Product, json.price, shop)
...
/**
 * Checks if product is in category structure where one (or many) categories are on ignore list.
 * Returns true if any of the categories is ignored, false if all categories are "clean"
 */
const categoryPathIsBanned = (async () => {
...
const fetchReviews = async (Product, i) => {
    ...
    for (let i = 0; i < response.data.reviews.length; i++) {
        ...
        // Ulkopuolisesta lähteestä, skippaa
